...
class User
{
	...
	public function follow_user($id,$type,$mybook_userid){
			...
			//save likes details
			$sql = "select following from likes where type='$type' && contentid = '$readbook_userid' limit 1";
			$result = $DB->read($sql);
			if(is_array($result)){
				...
				if(!in_array($id, $user_ids)){
					...
					$sql = "update likes set following = '$likes_string' where type='$type' && contentid = '$readbook_userid' limit 1";
					$DB->save($sql);
					...
					//add notification
					add_notification($_SESSION['readbook_userid'],"follow",$single_post);
				}else{
				...
				$sql = "insert into likes (type,contentid,following) values ('$type','$readbook_userid','$following')";
				$DB->save($sql);
